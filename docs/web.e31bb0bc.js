...
var dataTableOpts = {
  data: {
    headings: ['Tahun', 'Harga', 'Link']
  }
}; // https://github.com/fiduswriter/Simple-DataTables

var dataTable = new simpleDatatables.DataTable(table, dataTableOpts);
...
categoryAxis.renderer.labels.template.location = 0.5; // categoryAxis.renderer.inside = true;
...
chart.cursor = new am4charts.XYCursor(); // chart.scrollbarX = new am4core.Scrollbar();
// chart.scrollbarY = new am4core.Scrollbar();

function fetchFile() {
  var url = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '/sources/mazda.csv';
  return fetch(url).then(function (data) {
    return data.text();
  }).then(function (data) {
    return _papaparse.default.parse(data, {
      // worker: true,
      header: true
    });
  });
...
fetchFile().then(function (csvObj) {
  var models = [];
  csvObj.data.forEach(function (item) {
...
document.querySelector('#model').addEventListener('change', function () {
  var formattedChartObj = {};
  var formattedTableArr = [];
  var selectedModel = document.querySelector('#model').value;

  for (var idx = 0; idx < csvData.length; idx++) {
    var item = csvData[idx];
    ...
    formattedTableArr.push([item.Year, formatter.format(itemPrice), item.URL]);
    ...
    var chartItem = formattedChartObj[item.Year];
  ...
  for (var year in formattedChartObj) {
...
},{"papaparse":"../node_modules/papaparse/papaparse.min.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
